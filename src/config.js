export const isProduction = (process.env.NODE_ENV === 'production')
export const contactEmail = Buffer.from('amFrdWIuc3Rhc3RueS5wdCtibG9nQGdtYWlsLmNvbQ==', 'base64').toString('ascii')
export const personalContactEmail = Buffer.from('amFrdWIuc3Rhc3RueS5wdEBnbWFpbC5jb20=', 'base64').toString('ascii')
export const gravatarEmail = Buffer.from('amFrdWIuc3Rhc3RueS5wdCtibG9nQGdtYWlsLmNvbQ==', 'base64').toString('ascii')
export const mailchimpURL = 'https://botanicus.us3.list-manage.com/subscribe/post?u=3ab289fe1aba79ac1725973d1&amp;id=524bfa9e85'

export const googleAnalyticsTrackingId = 'UA-138694948-1'
