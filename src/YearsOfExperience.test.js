...
it('requires startYear to be present in the config', () => {
  expect(config.startYear).toEqual(2008)
})
...
it('renders <abbr> tag with title and children', () => {
  ...
  const yearsInBusiness = new Date().getFullYear() - config.startYear

  expect(object.type).toEqual('abbr')
  expect(object.props.title).toEqual(`From ${config.startYear} on`)
